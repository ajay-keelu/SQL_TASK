...
SELECT title,COUNT(*) AS 'Number of employees'  FROM [Ajay_task_6].dbo.Employee GROUP BY title ORDER BY COUNT(title);
...
SELECT title,AVG(salary) AS Average  FROM [Ajay_task_6].dbo.Employee GROUP BY title;

-- What is the average salary of employees excluding Freshers
SELECT AVG(salary) AS Average FROM [Ajay_task_6].dbo.Employee WHERE title <> 'Fresher';

--  What is the average age of employees of each unique title
SELECT title,AVG(age) AS Average  FROM [Ajay_task_6].dbo.Employee GROUP BY title;

-- In the age range of 25 to 40 get the number of employees under each unique title.
SELECT title,COUNT(*) AS 'Number of employees'  FROM [Ajay_task_6].dbo.Employee WHERE age BETWEEN 25 AND 40 GROUP BY title ;

-- Show the average salary of each unique title of employees only if the average salary is not less than 25000
SELECT title,AVG(salary) AS Average  FROM [Ajay_task_6].dbo.Employee GROUP BY title HAVING AVG(salary) > 25000;

-- Show the sum of ages of each unique title of employee only if the sum of age is greater than 30
SELECT title,SUM(age) FROM [Ajay_task_6].dbo.Employee GROUP BY title HAVING SUM(age) > 30;